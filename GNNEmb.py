...
parser.add_argument('--abl', action='store_true', default=False)

parser.add_argument('--optruns', type=int, default=100)
parser.add_argument('--path', type=str, default="EmbMy/") # Emb/
# parser.add_argument('--name', type=str, default="opt")
parser.add_argument('--name', type=str, default="ppi_bp")
...
def work(hidden_dim, conv_layer, dropout, jk, lr, batch_size):
    ...
    '''
    trn_loader = loader_fn(trn_dataset, batch_size) # 构建训练数据集
    val_loader = tloader_fn(val_dataset, val_dataset.y.shape[0]) # 构建验证数据集
    outs = []
